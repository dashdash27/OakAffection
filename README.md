...
- **Backend**: Реализован по паттерну **Application Factory** с использованием Blueprints.
- **Database**: **PostgreSQL**, правление миграциями через **Alembic**, оптимизация запросов через **SQLALchemy**.
- **Infrastructure**:
    - Веб-сервер: **Nginx** (Reverse Proxy, SSL).
    - WSGI: **Gunicorn**.
    - OS: **Ubuntu/Linux**.
    - CI/CD: **Manual CD** pipeline через защищенные ветки **Git (GitFlow)**.
    - **Тестовый контур** на поддомене с http-авторизацией.
- **Frontend**: Чистый Vanilla **JavaScript** (Fetch API, LocalStorage). Подход **Mobile First** и оптимизация ресурсов (**Lazy Loading**), шаблоны **Jinja**.
